...
public class DefaultQueuePollCommand<T> implements QueuePollCommand<MongoContext<T>, T> {
    ...
    public DefaultQueuePollCommand(boolean asc, IsEmptyCommand<MongoContext<T>> isEmptyCommand) {
        this.asc = asc;
        this.isEmptyCommand = isEmptyCommand;
    }
    
    @Override
    public T poll(MongoContext<T> ctx) {
        DBObject dbObject;
        ...
        dbObject = ctx.getCollection().findAndModify(null, null, asc ? ORDER_BY_ASC : ORDER_BY_DESC, true, null, false,
                false);
        return ctx.getSerializer().toElement(dbObject);
    }
