...
public class DefaultMapClearCommand<K, V> implements ClearCommand<MongoMapContext<K, V>> {
    
    private static final BasicDBObject EMPTY_BASICDBOBJECT = new BasicDBObject();
    
    @Override
    public void clear(MongoMapContext<K, V> ctx) {
        ctx.getCollection().remove(EMPTY_BASICDBOBJECT);
    }
