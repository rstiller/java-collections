...
public class DefaultContainsCommand<T> implements ContainsCommand<MongoContext<T>, T> {
    ...
    @Override
    public boolean contains(MongoContext<T> ctx, T object) {
        return ctx.getCollection().count(ctx.getSerializer().toDBObject(object, equalFunctions, negate)) > 0;
    }
