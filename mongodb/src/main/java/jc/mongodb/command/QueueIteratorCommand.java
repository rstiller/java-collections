...
import jc.mongodb.AbstractMongoCloseableIterator;
import jc.mongodb.MongoContext;

import com.mongodb.BasicDBObject;

public class QueueIteratorCommand<T> implements IteratorCommand<MongoContext<T>, T> {
    ...
    @Override
    public CloseableIterator<T> iterator(MongoContext<T> ctx) {
        return new AbstractMongoCloseableIterator<T>(ctx.getCollection().find()
                .sort(asc ? ORDER_BY_ASC : ORDER_BY_DESC), ctx.getSerializer()) {
        };
    }
